import pandas as pd

dosya = pd.read_excel("ANA_TABLO.xlsx") #, index_col=0) 
df_ana= pd.DataFrame(dosya,columns=["KURUM_KODU", "TİPİ","İLÇE","OKUL_KURUM_ADI","DERSLİK_SAYISI"])
# print("ANA_TABLO\n", df_ana)

dosya = pd.read_excel("BİNA_KULLANIM.xlsx") #, index_col=0)
df_bina = pd.DataFrame(dosya,columns=["KURUM_KODU", "TİPİ","İLÇE","KULLANIM_ALANI","OKUL_KURUM_ADI","SAYISI"])  
# print("BİNA_KULLANIM\n", df_bina)

# df1 = df_bina[df_bina["KULLANIM_ALANI"] == "8- TOPLAM DERSLİK SAYISI"]    # bu kod ile elde edeceğimiz sonucu "lambda" fonksiyonu ile yazmak istersek, aşağıdaki kod ile aynı sonucu elde edebiliriz.
df1 = df_bina.loc[lambda df_bina : (df_bina["KULLANIM_ALANI"] == "8- TOPLAM DERSLİK SAYISI")]
df1 = df1.groupby("KURUM_KODU").sum()["SAYISI"]
# print(df1)

# ### lambda fonksiyonu ile koşullu şecim örnekleri;
# print(df_bina.loc[lambda df_bina : (df_bina["KULLANIM_ALANI"] == "8- TOPLAM DERSLİK SAYISI") & (df_bina["KURUM_KODU"] == 16)])   # birden fazla koşul
# print(df_bina.loc[lambda df_bina : (df_bina["KULLANIM_ALANI"] == "8- TOPLAM DERSLİK SAYISI")])   # tek koşul

df_ana = df_ana.set_index("KURUM_KODU") # indeks değerleri ayarlanıyor.
# print(df_ana)

df_ana["DERSLİK_SAYISI"] = df1
# print(df_ana)

df_ana.to_excel("ANA_TABLO_SONUC.xlsx")
